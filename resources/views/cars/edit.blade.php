...
@section('content')
   ...
   @method('PUT')
    ...
    <div class="mb-3">
            ...
            <input type="text" class="form-control @error('year_release') is-invalid @enderror" id="year_release" name="year_release" value="{{ old('year_release', $car->year_release) }}">
            @error('year_release')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
    </div>
    <div class="mb-3">
            @foreach($optionals as $optional)
                ...
                @else
                    <input id="optionals_{{$optional->id}}" @if ($car->optionals->contains($optional->id)) checked @endif type="checkbox" name="optionals[]" value="{{$optional->id}}">
                @endif                
